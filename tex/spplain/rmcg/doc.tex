...
\def\cm#1fonts{\def\1{#1}\plainfont
 \ifx\1\x@ \def\1{cm}\else\def\1{\romannumeral#1}\fi
 \csname\1fonts\endcsname \csname\1titles\endcsname
 \font\Euromr=feymr10 at #1pt\def\EUR{\hbox{\Euromr e}}}
...
% EUR (€) is unicode 20-AC, UTF-8 E2-82-AC (3 bytes)
% Trade Mark (TM) is unicode 21-22, UTF-8 E2-84-A2 (3 bytes)
\expandafter\def\csname E2^^82^^ac\endcsname{\EUR}
\expandafter\def\csname E2^^84^^a2\endcsname{\TradeMark}
\catcode`\^^e2=13
\def^^e2{\begingroup\def\next{E2}\deactivate\dohigh\g@tnexttwo}
\def\g@tnexttwo#1#2{\csname \next#1#2\endcsname\endgroup}

% EUR (€) is A4 in ISO 8859-15 
\catcode`\^^a4=13 \def^^a4{\EUR}

% Euro font  oblique              right
\font\eurobo=feybo10 \font\eurobr=feybr10 % bold
\font\euromo=feymo10 \font\euromr=feymr10 % medium
\def\EUR{\hbox{\euromr e}}
...
\def\MTfigure(#1,#2,#3);{\catcode`\"=12
 \ifx\MT\undefined \input metatex \MTline{input delay;}\fi
...
\def\beginalign{$$\displayindent=\parindent \offinterlineskip
  ...
  \def\rule{\noalign{\vskip0pt\hrule\vskip1pt}}\let\\=\cr
...
\def\rmcglayout{\vfill\break \xiifonts
 \parskip=\medskipamount
 \nopagenumbers \pdfsetpagesize \advance\vsize -30pt
 \headline={\rm \Author \quad \todayiso\hfil
  \header\quad{\tt \jobname}\quad \PageSTR\ \Folio}%
 \def\makeheadline{\vbox to 30pt{\line{\the\headline}%
  \kern 1pt \hrule height 1pt\vfil}\nointerlineskip}}

\def\Author{{\tt r.casares@ieee.org}}
\def\eaddress{\ifcase\pdfoutput {\tt r.casares@ieee.org}\else
  \pdfstartlink attr{/Border [0 0 0]}
   user{/Subtype /Link /A << /Type /Action
    /S /URI /URI (mailto:r.casares@ieee.org) >>}%
   {\tt r.casares@ieee.org}\pdfendlink
 \fi}
\def\docinfo{{\tt\jobname\ \copyright\ r.casares@ieee.org \todayiso}}
%\def\infodoc{{\tt\jobname\ \copyright\ r.casares@ieee.org \todayiso}}
\let\infodoc=\docinfo
...
\def\letterlayout{\vfill\break
 ...
 \font\twelvesfdc=cmssdc10 scaled1200 \font\tensfdc=cmssdc10
...
\def\booklayout{\vfill\break \cmfonts
 \headline={\hfil}
