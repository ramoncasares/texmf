...
\def\infodoc{\begingroup
  ...
  \def\new/{}\def\wen/{}\def\del/{}\def\led/{}\def\mod[##1<##2]{##1}%
...
\def\colornew{Red}\def\colordel{White}

\def\marginalstar{\vbox to 0pt{\kern-9pt
 \llap{\tt\truecolor{\colornew}*\endcolor\quad}\vss}}

\def\new/{\truecolor{\colornew}}
\def\wen/{\endcolor\ifvmode\nointerlineskip\marginalstar
 \else\vadjust{\marginalstar}\fi}

\def\del/{\begingroup\uncatcodespecials
 \ifvmode \let\next=\DELv \else \let\next=\DELh \fi\next}
{\catcode`\|=0 \catcode`\\=12
 |long|gdef|DELv#1\led/{|marginalstar|kern-|baselineskip|endgroup}
 |long|gdef|DELh#1\led/{|vadjust{|marginalstar}|truecolor{|colornew}|not|endcolor|endgroup|ignorespaces}}
\def\led/{\errmessage{Error! Unnested led.}}

% \def\mod[#1<{\leavevmode\def\m@@{#1}\relax
%  \begingroup \uncatcodespecials\deactivatehigh\utfon\out{pdf}\doextrapdf\relax\m@d}
% \def\m@d#1]{\special{pdf: bann << /Subtype /Link /Border [0 0 0]
%  /A << /Type /Action /S /JavaScript
%  /JS (app.alert({cTitle: "mod", cMsg: "#1 -> \m@@", nIcon: 3, nType: 0 });) >> >>}\endgroup
%  \truecolor{Red}\ifx\m@@\empty\rlap{---}\let\next\ignorespaces\else\m@@\let\next\relax\fi\endcolor
%  \special{pdf: eann}\vadjust{\marginalstar}\next}
% \def\m@d#1]{\special{pdf: bann << /Subtype /Link /Border [0 0 0]
...
\def\mod[#1<#2]{\begingroup\def\1{#1}\def\2{#2}%
 \ifx\2\empty\else \truecolor{\colornew}\rlap{---}\endcolor\truecolor{\colordel}\rlap{#2}\endcolor \fi
 \ifx\1\empty\else \truecolor{\colornew}#1\endcolor \fi
 \ifvmode \marginalstar\kern-\baselineskip \else \vadjust{\marginalstar}\fi\endgroup}
