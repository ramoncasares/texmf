...
\ifMTf@le \message{Second TeX pass} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 \else   % MetaPost has written auxiliar.log
  ...
  \else
   ...
   %%\input supp-pdf.tex % older Context
   %%\let\oldfinMPgrap=\finishMPgraphic % to avoid centering
   %%\def\finishMPgraphic{\xdef\MPxoffset{0}\xdef\MPyoffset{0}\oldfinMPgrap}%
   %%\def\MTchar{\convertMPtoPDF{auxiliar.\number\MTn@}{1}{1}}%
  \fi
...
\MT: enddef;
\MT:
\MT: vardef thebblabel@#(expr s,z) =  % Position bbox s near z
\MT:   save p; picture p;
\MT:   if picture s:  p=s
\MT:   else:    p = s infont defaultfont scaled defaultscale
\MT:   fi;
\MT:   bbox p shifted (z + labeloffset*laboff@# -
\MT:      (labxf@#*lrcorner p + labyf@#*ulcorner p
\MT:        + (1-labxf@#-labyf@#)*llcorner p
\MT:      )
\MT:   )
\MT: enddef;
\MT:
\MT: def xlabel text t =
\MT:  erase fill thebblabel t;
\MT:  draw thelabel t;
\MT: enddef;
\MT:
