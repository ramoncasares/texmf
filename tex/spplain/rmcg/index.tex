...
\def\dvipscol@r#1{\special{color push cmyk \csname cmyk#1\endcsname}}
\def\dvipsendcol@r{\special{color pop}}

\newcount\col@rstack
\def\pdfcol@r#1{\pdfcolorstack\col@rstack push {\csname cmyk#1\endcsname k}}
\def\pdfendcol@r{\pdfcolorstack\col@rstack pop}
...
% \color{Red}#1\endcolor \pdfcode\pdfendlink\pdfendcode}

{\catcode`\#=12 \xdef\hash{#}}

\def\pdfg@to#1#2{\leavevmode
 \pdfstartlink attr{/F 4 /Border [0 0 0]} goto \typelink{#2}%
 \color{Red}#1\endcolor\pdfendlink}
\def\dvipsg@to#1#2{\leavevmode
 \special{html:<a href=\string"\hash\typelink{#2}\string">}%
 \color{Red}#1\endcolor\special{html:</a>}}
...
\ifcase\pdfoutput
 ...
 \def\colors{\let\color=\dvipscol@r\let\endcolor=\dvipsendcol@r}
 \def\links{\let\goto=\dvipsg@to\let\dest=\dvipsd@st}
\else
 \let\pdfcode=\relax \let\pdfendcode=\relax
 \col@rstack=\pdfcolorstackinit{\pdfliteral{0 0 0 1 k}}
 \def\colors{\let\color=\pdfcol@r\let\endcolor=\pdfendcol@r}
 \def\links{\let\goto=\pdfg@to\let\dest=\pdfd@st}
\fi

\def\nocolor{\def\color##1{\relax}\def\endcolor{\relax}}
\nocolor % default value
\def\nolinks{\def\goto##1##2{\relax}\def\dest{\relax}}
\links % default value
...
\def\files{\begingroup \stringate\dohigh\stringaccents \let^^c2\empty
 ...
 \ifauxf
  ...
  \pdft@toc0
  \ifcase\pdfoutput \let\tocline=\outline@dvips
  \else \let\tocline=\outline@pdf \fi
  \input auxiliar.toc
 \fi
 \endgroup
