...
\command\reference Knuth (1984): Starts a reference destination
with the information about the book, article, or document.
Macro "\reference" is delimited thus: "#1 (#2)",
meaning that the space and the pair of parenthesis are mandatory.
The real "label" of "\reference Name (MCDU)" is "NameMCDU".
...
It is also possible to take advantage of "RCbiblio.tex",
my bibliographic database.
\command\references: Makes the equivalent to
 "\xsection{References}" and
 starts a bibliographic area, where it is enough to
 write "\item Knuth (1984)" to include all the
 bibliographic information of the item in the database,
 exactly as if "\reference" were used,
 which also works inside the area.
\command\item Knuth (1984): Asks to include all the
 bibliographic information of the item from "RCbiblio.tex".
\command\xitem Casares (PT) = Casares (P): Uses reference
 "Casares (PT)" to make cites, but takes the information
 from the database item labeled "Casares (P)".
\command\endreferences: Closes the bibliographic area.
...
\command\mod[new<old]: syntactic sugar for "\diff{new}{old}".

% \continuepar % No longer correct
...
has to be written {\em before} the command "\input RCstyle":
"\def\doextrasave{\def\diff##1##2{##1}\def\new/##1\wen/{##1}%
 \def\del/##1\led/{}}"
...
\xitem Casares (PT) = Casares (P)

\reference Knuth (1984)
Donald E. Knuth,
\book{The {\TeX}book},
