...
\def\file{\medbreak \begingroup
 \catcode`\~=12 \catcode`\"=12 \catcode`\\=12 \xfile}
\def\xfile#1{%
 \hbox{\indent\index{#1}1\vbox{\hbox{\strut\Blue\tt#1\Black}\hrule}}%
 \nobreak\smallskip\catcode`\|=12
 \let\verbatimoptions=\listingoptions
 \listing{#1}\smallbreak\endgroup}

\catcode`\"=13 \def"{\verb"}
\catcode`\|=13
\def|{\penalty100\smallskip\verb|\aftergroup\endgraf}
...
\point esta presentación general,
\point una segunda parte sobre ejecutables (\latin{scripts}),
\point otra tercera sobre formatos,
\point la cuarta sobre macros,
\point y la última sobre tipos (fuentes, o \latin{fonts}).
